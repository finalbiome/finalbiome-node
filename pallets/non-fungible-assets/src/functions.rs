...
use pallet_support::{AssetCharacteristic, Locker};
...
impl<T: Config> Pallet<T> {
  ...
  pub(crate) fn do_burn(
    ...
    Assets::<T>::try_mutate_exists(class_id, asset_id, |maybe_details| {
      ...
      if let Some(check_owner) = maybe_check_owner {
          ...
          &asset_details.owner == check_owner,
      ...
      Classes::<T>::try_mutate(&class_id, |maybe_class_details| -> DispatchResult {
        let class_details = maybe_class_details
          .as_mut()
          .ok_or(Error::<T>::UnknownClass)?;
        class_details.instances = class_details
          .instances
          .checked_sub(1)
  ...
  pub fn get_class_details(class_id: &NonFungibleClassId) -> DispatchResultAs<ClassDetailsOf<T>> {
  ...
  pub(crate) fn set_lock(
    ...
    match details.locked {
      ...
      _ if details.locked == origin => Ok(LockResultOf::<T>::Already(details)),
      _ => Err(Error::<T>::Locked.into()),
    }
  }
