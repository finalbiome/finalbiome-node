...
use pallet_support::{
  types_nfa::ClassDetails, DispatchResultAs, FungibleAssetBalance, FungibleAssetId, LockResult,
  Locker, NonFungibleAssetId, NonFungibleClassId,
};
use sp_core::H256;
use sp_runtime::{
  testing::Header,
...
impl pallet_support::traits::FungibleAssets<u64> for FAPallet {
  ...
  fn mint_into(
    ...
    if asset == 12.into() {
      // test do_onboarding_works, onboarding_works
...
impl pallet_support::traits::NonFungibleAssets<u64, u32> for NFAPallet {
  fn mint_into(class_id: &NonFungibleClassId, who: &u64) -> DispatchResultAs<NonFungibleAssetId> {
    if class_id == &10.into() {
      // test do_airdrop_nfa_works
      return Ok(10.into());
    }
    if class_id == &11.into() {
      // test do_airdrop_nfa_works
  ...
  fn get_class(_class_id: &NonFungibleClassId) -> DispatchResultAs<ClassDetails<u64>> {
    todo!()
  }
  fn burn(
    _class_id: NonFungibleClassId,
