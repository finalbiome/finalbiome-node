	...
	T: frame_system::Config,
{
	fn random(_subject: &[u8]) -> (Output, T::BlockNumber) {
		...
		let block_number: u32 = frame_system::Pallet::<T>::block_number().try_into().unwrap_or_default();
		(
...
fn burn_from(
	...
	fn mint_into(_asset: FungibleAssetId, _who: &u64, _amount: FungibleAssetBalance) -> sp_runtime::DispatchResult {
		todo!()
	}
}
