...
use pallet_support::{
  ...
  AssetCharacteristic, ClassDetailsOf, DefaultListLengthLimit, GamerAccount, LockedAccet,
  MechanicIdOf,
};
...
fn do_do_upgrade_bet_two_rounds_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_ok!(MechanicsModule::upgrade(
      Origin::signed(who),
      org,
      upgrage_data
    ));
