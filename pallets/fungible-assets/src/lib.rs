  ...
  impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
    fn build(&self) {
      ...
      for (asset_id, organization_id, name, top_upped, cup_global, cup_local) in &self.assets {
        ...
        let cup_global = cup_global.as_ref().map(|amount| CupFA { amount: *amount });
        let cup_local = cup_local.as_ref().map(|amount| CupFA { amount: *amount });
        let ad = AssetDetailsBuilder::<T>::new(organization_id.clone(), name.to_vec())
          .unwrap()
          .top_upped(top_upped)
          .unwrap()
