...
impl<T: Config> Pallet<T> {
    ...
    Assets::<T>::try_mutate(id, |maybe_details| -> DispatchResult {
      ...
      Accounts::<T>::try_mutate(beneficiary, id, |maybe_account| -> DispatchResult {
        match maybe_account {
          ...
          maybe_account @ None => {
            *maybe_account = Some(AssetAccount {
              balance: amount,
  ...
  pub fn top_upped_asset_add(id: &AssetId) -> DispatchResult {
    let mut current_topupped = match <TopUppedAssets<T>>::try_get() {
      Ok(curr) => curr,
      Err(_) => <WeakBoundedVec<AssetId, T::MaxTopUppedAssets>>::try_from(Vec::new())
        .map_err(|()| Error::<T>::MaxTopUppedAssetsReached)?,
    };
  ...
  pub fn top_upped_asset_remove(id: &AssetId) {
    let mut current_topupped = match <TopUppedAssets<T>>::try_get() {
      Ok(curr) => curr,
      Err(_) => return,
    };
